...
GitHub Pages 是静态网站，搜索需要后端代理。`*.workers.dev` 在国内经常无法访问，推荐使用 **腾讯云云函数 + 函数 URL**。

> **注意**：腾讯云 API 网关已于 2025-06-30 停止服务，请使用 **函数 URL**，不要创建 API 网关触发器。

## 架构

```
GitHub Pages（前端）  →  腾讯云云函数（函数 URL）  →  博查 API
```

---

## 方案一：腾讯云云函数 + 函数 URL（推荐）
...
```
https://1250000000-abc123def.ap-guangzhou.tencentscf.com/api/search?q=测试&cat=all
```
...
API 网关已下线，请改用 **函数 URL**（第五步）。

### `ERR_CONNECTION_TIMED_OUT`

说明当前网络访问不了代理地址。若用的是 `workers.dev`，请改用本指南的腾讯云函数 URL。
